...
using CardDto = Stations.DataProcessor.Dto.Import.CardDto;
using TripDto = Stations.DataProcessor.Dto.Import.TripDto;
...
namespace Stations.DataProcessor
{
    public static class Deserializer
    {
        ...
        {
            var deserializedSeatingClasses = JsonConvert.DeserializeObject<SeatingClassesDto[]>(jsonString);
            var sb = new StringBuilder();
            var validData = new List<SeatingClass>();
            foreach (var seatingClassDto in deserializedSeatingClasses)
            {
                ...
                if (validData.Any(e =>
                    e.Name == seatingClassDto.Name || e.Abbreviation == seatingClassDto.Abbreviation))
                {
                    sb.AppendLine(FailureMessage);
                    continue;
                }
                var seatingClass = Mapper.Map<SeatingClass>(seatingClassDto);
                validData.Add(seatingClass);
                sb.AppendLine(String.Format(SuccessMessage, $"{seatingClassDto.Name}"));
            }
            context.SeatingClasses.AddRange(validData);
            context.SaveChanges();
        ...
        {
            var deserializedTrains = JsonConvert.DeserializeObject<TrainsDto[]>(jsonString, new JsonSerializerSettings()
            {
        ...
        public static string ImportTrips(StationsDbContext context, string jsonString)
        {
            ...
            foreach (var tripDto in deserializedTrips)
            {
                ...
                var originStation = context.Stations.SingleOrDefault(e => e.Name == tripDto.OriginStation);
                var destinationStation = context.Stations.SingleOrDefault(e => e.Name == tripDto.DestinationStation);
        ...
        public static string ImportCards(StationsDbContext context, string xmlString)
        {
            ...
            foreach (var cardDto in deserializedCards)
            {
                ...
                var customerard = new CustomerCard()
                {
                    Name = cardDto.Name,
                    Type = cardType,
                    Age = cardDto.Age
                };
                validCards.Add(customerard);
        ...
        public static string ImportTickets(StationsDbContext context, string xmlString)
        {
            ...
            var deserializedTickets =
                (TicketDto[]) serializer.Deserialize(new MemoryStream(Encoding.UTF8.GetBytes(xmlString)));
            ...
            foreach (var ticketDto in deserializedTickets)
            {
                ...
                var seatExists = trip.Train.TrainSeats
                    .SingleOrDefault(s =>
                        s.SeatingClass.Abbreviation == seatingClassAbbreviation && quantity <= s.Quantity);
                if (seatExists == null)
                {
                    sb.AppendLine(FailureMessage);
                    continue;
